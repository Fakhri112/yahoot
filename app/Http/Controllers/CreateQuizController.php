...
class CreateQuizController extends Controller
{
    ...
    public function submitCreateQuizData(Request $request)
    {
        ...
        $quiz_detail->quiz_title = $quiz_detail_data['quiz_title'];
        ...
        return Redirect::to('/user/' . auth()->user()->id)->with('message', 'Quiz Created!');
    }
    ...
    public function submitUpdateQuizData(Request $request)
    {
        ...
        foreach ($question_datas as $key => $question_data) {
            ...
            preg_match("/\/user_quiz\/" . auth()->user()->id . "\/[a-f0-9]+\//", $quiz_detail_data['prev_thumbnail'], $get_image_folder_id);
        ...
        return Redirect::to('/user/' . auth()->user()->id)->with('message', 'Quiz Updated!');;
